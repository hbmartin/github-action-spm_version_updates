...
RSpec.describe SpmChecker, "#check_for_updates" do
  ...
  it "reports newer versions for up to next major" do
    ...
    expect(check_for_update_messages(fixture("UpToNextMajor"))).to eq(["Newer version of kean/Nuke: 12.1.7"])
  end

  it "returns a structured result with string-keyed update records", :aggregate_failures do
    allow(GitOperations).to receive(:version_tags).and_return(versions("12.1.7", "12.1.6"))

    result = checker.check_for_updates(fixture("UpToNextMajor"))

    expect(result).to be_a(described_class::Result)
    expect(result.updates.first).to include(
      "message" => "Newer version of kean/Nuke: 12.1.7",
      "package" => "kean/Nuke",
      "current_version" => "12.1.6",
      "available_version" => "12.1.7"
    )
    expect(result.parse_warnings).to eq([])
  end
  ...
  it "queries git with the original scheme-bearing URL, not the normalized match key" do
    ...
    check_for_update_messages(fixture("UpToNextMajor"))
  ...
  it "reports newer versions for exact constraints when check_when_exact is set" do
    ...
    expect(check_for_update_messages(fixture("ExactVersion"))).to eq(
  ...
  it "reports newer versions within a version range" do
    ...
    expect(check_for_update_messages(fixture("VersionRange"))).to eq(["Newer version of kean/Nuke: 12.1.7"])
  end
  ...
  it "reports newer commits for branch-pinned dependencies" do
    ...
    expect(check_for_update_messages(fixture("Branch"))).to eq(
  ...
  it "does not report commit/revision-pinned dependencies by default" do
    expect(check_for_update_messages(fixture("Commit"))).to eq([])
  end
  ...
  it "reports the above-maximum version when report_above_maximum is set" do
    ...
    expect(check_for_update_messages(fixture("UpToNextMajor"))).to eq(
  ...
  it "reads version=1 Package.resolved files" do
    ...
    expect(check_for_update_messages(fixture("PackageV1"))).to eq(["Newer version of gonzalezreal/NetworkImage: 3.1.3"])
  end

  it "handles ssh and .git repository URLs" do
    allow(GitOperations).to receive(:version_tags).and_return(versions("12.1.7", "12.1.6"))

    expect(check_for_update_messages(fixture("MangledUrl"))).to eq(["Newer version of kean/Nuke: 12.1.7"])
  end
  ...
  it "merges both Package.resolved locations" do
    ...
    expect(check_for_update_messages(fixture("AlsoHasXcworkspace"))).to eq(
      [
  ...
  it "raises when no Package.resolved is present" do
    expect {
      check_for_update_messages(fixture("NoPackagesResolved"))
  ...
  it "warns when Package.resolved has pins but the Xcode project has no remote package references", :aggregate_failures do
    ...
    expect {
      expect(check_for_update_messages("App.xcodeproj")).to eq([])
    }.to output(
      a_string_including(
